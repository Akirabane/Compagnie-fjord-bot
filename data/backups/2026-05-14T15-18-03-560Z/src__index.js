...
import { approveRequest, refuseRequest, processRequest } from './utils/code-agent.js';
...
import db, { stmts as dbStmts } from './db/database.js';
...
// ── Embed constant du salon d'aide ────────────────────────────────────────────
const AIDE_EMBEDS = [
  {
    title: '📖 Commandes — Membres & Visiteurs',
    color: 0xC9A84C,
    fields: [
      ...
      { name: '🍺 `/recette`',    value: 'Rechercher une recette de craft, par profession ou par nom.' },
  ...
  },
  {
    ...
    fields: [
      ...
      { name: '💬 `/recap`',       value: 'Voir ou effacer votre historique de conversation avec l\'Intendant IA.' },
  ...
  {
    title: '🛡️ Commandes Marchands & Administration',
    color: 0x1A3A5C,
    fields: [
      ...
      { name: '🔔 `/alertes`',   value: 'Configurer le salon des alertes Intelligence Économique.' },
      { name: '💬 `/forum`',     value: 'Configurer les forums Discord pour commandes acheteurs et offres vendeurs.' },
...
async function postAideEmbed(client) {
  try {
    ...
    for (const embedDef of AIDE_EMBEDS) {
      await channel.send({
        embeds: [{
          ...embedDef,
          footer: { text: '⚓ La Compagnie du Fjord — Les 3 Routes • Fjordheim, Empire de Skanor' },
          timestamp: new Date().toISOString(),
        }],
      });
    }
    console.log('[aide embed] Embeds de commandes postés dans le salon aide.');
  } catch (e) { console.error('[aide embed]', e); }
}

async function recoverStuckRequests(client) {
  // Remet en pending les demandes bloquées en "processing" suite à un crash/restart
  db.exec("UPDATE code_requests SET status='pending' WHERE status='processing'");
  const pending = dbStmts.codeReqPending.all();
  for (const req of pending) {
    processRequest(req.id)
      .then(async ({ parsed, diff, hasError }) => {
        ...
        if (!owner) return;
        if (hasError) {
          return owner.send({ content: `⚠️ Demande #${req.id} refusée par l'IA : ${diff}` });
        }
        const { ActionRowBuilder, ButtonBuilder, ButtonStyle } = await import('discord.js');
        const row = new ActionRowBuilder().addComponents(
          new ButtonBuilder().setCustomId(`modifier_approve:${req.id}`).setLabel('✅ Approuver & Appliquer').setStyle(ButtonStyle.Success),
          new ButtonBuilder().setCustomId(`modifier_refuse:${req.id}`).setLabel('❌ Refuser').setStyle(ButtonStyle.Danger),
        );
        const { embedBase, COULEURS } = await import('./utils/embeds.js');
        const embed = embedBase(
          `🛠️ Demande de modification #${req.id}`,
          `**Demandeur :** ${req.requester_name}\n\n**Demande :**\n> ${req.description}\n\n${diff}`,
          COULEURS.bleu
        ).addFields({ name: '⚠️ Action requise', value: 'Approuvez ou refusez cette demande.' });
        owner.send({ embeds: [embed], components: [row] });
      })
      .catch(err => console.error(`[modifier] Erreur récupération #${req.id}:`, err.message));
  }
}
...
client.once('clientReady', async () => {
  ...
  recoverStuckRequests(client).catch(e => console.error('[modifier recover]', e));
});
  ...
  try {
    ...
    if (interaction.isButton()) {
      ...
      if (id.startsWith('modifier_approve:') || id.startsWith('modifier_refuse:')) {
        ...
        try {
          const { summary, files, restarted, backupDir } = await approveRequest(reqId, interaction.user.username);
          const embed = new EmbedBuilder()
            .setColor(0x3dd68c)
